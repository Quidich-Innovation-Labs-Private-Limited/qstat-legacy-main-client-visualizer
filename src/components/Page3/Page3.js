...
function Page3(props) {
  ...
  const [basket, setBasket] = useState(
    new Array(Object.keys(selectedBalls).length).fill([])
  );
  ...
  const removeList1 = (data) => {
    ...
    let newplayerList = player.filter(
      (item) =>
        item.identities != data.identities &&
        item.homo_track[0] != data.homo_track[0]
    );
    let originalData = selectedBalls[Object.keys(selectedBalls)[0]].players;
    let originalItem = originalData.find(
      (item) => item.identities == data.identities
    );
  ...
  const changeList1 = (data) => {
    if (swich.status && swich.item.list === 1) {
      ...
      let reItem = originalData.find(
        (item) => item.identities == swich.item.value
      );
      ...
      setPlayer([
        ...player,
        selectedBalls[Object.keys(selectedBalls)[0]].players.find(
          (item) => item.identities == data.identities
        ),
      ]);
    }
  ...
  const changeList2 = (data) => {
    if (swich.status && swich.item.list === 2) {
      ...
      setPlayer([
        ...player,
        selectedBalls[Object.keys(selectedBalls)[1]].players.find(
          (item) => item.identities == data.identities
        ),
      ]);
    }
  ...
  const handleClick = () => {
    ...
    console.log(
      "Other Players",
      selectedBalls
        ? selectedBalls[Object.keys(selectedBalls)[0]].players.filter(
            (val) => !player.includes(val)
          )
        : ""
    );
    const list1id = list1.map((item) => {
    ...
    if (Object.values(selectedBalls).length !== 2) {
      ...
      if (player.length === 0) {
        ...
        unselected_players = selectedBalls
          ? selectedBalls[Object.keys(selectedBalls)[0]].players.filter(
              (val) => !list1id.includes(val.identities)
            )
          : "";
    }
    ...
    axios({
      method: "post",
      url: "http://localhost:8081/launch",
      data: launchdata, // you are sending body instead
      headers: {
  ...
  return (
    <>
      <div className="tableheaderpage3">
        ...
        <div className="selectdivPage3">
          ...
          <div>
            <Select className="select-div-item" placeholder="Inings" />
          </div>
          <button className="search-button">Search</button>
        </div>
      </div>

      <div className="main-contentpage3">
        {Object.values(selectedBalls).length !== 2 ? (
          Object.values(selectedBalls).map((item) => {
            return (
              <>
                <Feild
                  ball={item}
                  index={Object.values(selectedBalls).indexOf(item)}
                  total={Object.values(selectedBalls).length}
                  player={player}
                  selectedFilter={selectedFilter}
                  selectFielder={changeList1}
                  selected={basket[Object.values(selectedBalls).indexOf(item)]}
                />
              </>
            );
          })
          ...
          <>
            <Feild
              ball={
                selectedBalls
                  ? selectedBalls[Object.keys(selectedBalls)[0]]
                  : {}
              }
              ball2={
                selectedBalls
                  ? selectedBalls[Object.keys(selectedBalls)[1]]
                  : {}
              }
              player={player}
              remove={removeList1}
              ...
              selectFielder2={changeList2}
              selected2={list2}
            />
          </>
        )}
      </div>
      {/* {player.length>0 ?<div className="pairspage3">
        <div className="pair-listpage3">{list1.length>0 ?list1.map(item=>{
          return( 
            <div className="pairpage3" style={swich.status && swich.item.value === item.identities && swich.item.list === 1? {backgroundColor:'white'}: {backgroundColor:colorCode(item.identities,list1)}} onClick={()=>{holdItem(item.identities, 1);}}>
              {swich.status && swich.item.value === item.identities && swich.item.list === 1 ? '.':  item.identities}
            </div>
          )
        }):''}</div>
        <div className="pair-listpage3">{list2.length>0 ?list2.map(item=>{
          return( 
            <div className="pairpage3" style={swich.status && swich.item.value === item.identities && swich.item.list === 2? {backgroundColor:'white'}: {backgroundColor:colorCode(item.identities,list2)}} onClick={()=>{holdItem(item.identities, 2);}}>
